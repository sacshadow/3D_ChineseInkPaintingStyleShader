...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata v) {
			...
			float rr = lerp(5, _RimRate, clamp(dis/_RemapCtrl.x*8,0,1));
	...
	Pass
	{
		...
		v2f vert (appdata v) {
			...
			o.mcrmd.xy = normalize(normalize(map.xy) + float2(-extend.y,extend.x)) * range * _OutlineCtrl.zw/10 * 0.5 + 0.5;
